...
prepareMetadata <- function(metadataTable, makeConditionsFrom = NULL, na.value = "") {
  ...
  out <- metadataTable |>
    ...
    tidyr::unite(col = Condition, dplyr::all_of(makeConditionsFrom)) |>
    dplyr::mutate(Condition = stringr::str_replace_all(Condition, pattern = " ", replacement = "_"),
...
runTxi <- function(metadata, dataset) {

  md <- metadata

  files <- c()
  dl_files <- list.files(glue::glue("rsem/{dataset}"), full.names = TRUE, pattern = glue::glue(".genes.results"))

 for (run in unique(md$Run)) {
    filename <- glue::glue("rsem/{dataset}/{run}.genes.results")
